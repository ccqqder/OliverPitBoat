...
A: 기기 성능에 따라 앱이 적절한 모델(Llama 3.2 또는 Qwen 2.5)을 추천합니다. 모델 크기는 모델에 따라 다르며, 수백 MB에서 수 GB까지 다양합니다. 다운로드 후 대화는 **완전 오프라인**으로 작동하며 인터넷이 필요하지 않습니다.
...
1. **AI 모델 로드 실패**: 선택한 모델에 충분한 여유 저장 공간이 있는지 확인하고 다운로드가 중단되지 않았는지 확인
